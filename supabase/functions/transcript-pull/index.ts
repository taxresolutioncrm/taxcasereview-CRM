...
async function resolveEmployee(userDb: any, user: any) {
  ...
  if (tenantErr || !tenantId) throw new Error('Could not resolve the active TaxRes office for this IRS session.')
  const { data, error } = await userDb.from('employees').select('tenant_id,perm_irs,email').eq('tenant_id', tenantId).ilike('email', email).limit(2)
