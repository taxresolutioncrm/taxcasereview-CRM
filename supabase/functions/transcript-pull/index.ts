...
async function resolveEmployee(userDb: any, user: any) {
  ...
  const { data, error } = await userDb.from('employees').select('tenant_id,perm_irs,email,caf,caf_number').ilike('email', email).limit(2)
...
async function resolveContext(service: any, pull: any, employee: any) {
  ...
  const caf = String(employee?.caf_number || employee?.caf || '').trim(); if (!caf) throw new Error('Practitioner CAF number is required for direct TDS.')
