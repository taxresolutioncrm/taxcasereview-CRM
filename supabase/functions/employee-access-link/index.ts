...
async function sendBrandedAccess(url:string,service:string,target:string,name:string,kind:'invite'|'recovery',accessLink:string){
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(mode==='readiness'){
      ...
      const targets=(staff||[]).slice(0,100)
      const results:any[]=await mapLimited(targets,10,async(emp:any)=>{
        const email=String(emp.email||'').trim().toLowerCase()
    ...
    if(mode==='prepare'){
      ...
      const results:any[]=await mapLimited(targets,10,async(emp:any)=>{
        const email=String(emp.email||'').trim().toLowerCase()
        if(!email || !/^\S+@\S+\.\S+$/.test(email)) return {id:emp.id,name:emp.name,email,status:'blocked',reason:'invalid_email'}
        if((counts.get(email)||0)>1) return {id:emp.id,name:emp.name,email,status:'blocked',reason:'duplicate_employee_email'}
        const targetRank=RANK[emp.access||'']||0
        if(actor.access!=='Super Admin' && targetRank>=actorRank && email!==String(actor.email||'').toLowerCase()){
          return {id:emp.id,name:emp.name,email,status:'blocked',reason:'protected_access'}
        }
        try{
          const status=await familyAccess(authHeader,email,'status')
          if(status?.last_sign_in_at) return {id:emp.id,name:emp.name,email,status:'already_ready',last_sign_in_at:status.last_sign_in_at}
          const family=await familyAccess(authHeader,email,'link')
          return {id:emp.id,name:emp.name,email,status:'prepared',kind:family.mode||'invite',access_link:family.access_link}
        }catch(e){
          return {id:emp.id,name:emp.name,email,status:'error',reason:e instanceof Error?e.message:String(e)}
        }
      })
    ...
    try{
      ...
      try{
        await sendBrandedAccess(url,service,target,emp.name||'',kind,accessLink)
        return json({
