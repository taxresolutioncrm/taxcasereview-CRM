...
need(`${helperDir}/background.js`, 'answer.tenantId === pair.tenantId', 'delivery only to the paired CRM tab while it is still in the same office')
need(`${helperDir}/background.js`, 'if (tabId == null) return // can\'t tell which IRS window', 'ambiguous downloads never sent')
need(`${helperDir}/mailbox.js`, "if (!isMailboxPage()) return", 'helper never scans or clicks on TDS request pages')
need(`${helperDir}/mailbox.js`, "if (form.querySelector('input[type=\"password\" i], select, textarea')) return null", 'only simple hidden-field attachment forms are replayed')
...
need(lib, 'if (sameTin.size !== 1 || !sameTin.has(candidates[0].client_id)) return null', 'auto-file only when exactly one client has that SSN/EIN ending')
need(lib, ".eq('tenant_id', tenantId).eq('raw_analysis->>file_sha256', key)", 'duplicate check scoped to this office')
need(lib, 'if (!a?.file_sha256) a = { ...a, file_sha256: await sha256File(file) }', 'every filing path records the PDF fingerprint')
need(pullUi, 'This CRM tab is now signed in to a different office', 'office re-checked right before filing')
