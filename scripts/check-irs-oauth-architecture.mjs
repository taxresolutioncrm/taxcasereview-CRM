...
console.log('Checking Step 1: Practitioner TDS sign-in + separate automated API authorization…')
need(session, "const IRS_TDS_URL = 'https://la.www4.irs.gov/esrv/tds/'", 'official IRS TDS URL')
need(session, 'openPractitionerTds', 'practitioner TDS launcher')
need(session, 'data-testid="irs-tds-sign-in"', 'practitioner TDS sign-in control')
need(session, 'Sign in to IRS TDS', 'practitioner TDS label')
need(session, 'Automated CRM delivery (separate path)', 'separate automated API status')
need(session, "action: 'begin-session'", 'separate automated API begin-session route')
need(session, 'Authorize Automated IRS API', 'separate automated API authorization control')
need(session, 'authorizationUrl', 'authorizationUrl used only for automated API authorization')
need(session, 'taxres-irs-tds-oauth', 'automated API callback message')
