...
const stripeCheckoutWebhook = read('supabase/functions/stripe-checkout-webhook/index.ts')
const stripeResolutionIntent = read('supabase/functions/stripe-resolution-fee-intent/index.ts')
const stripeResolutionConfirm = read('supabase/functions/stripe-resolution-fee-confirm/index.ts')
...
must(employees.includes("window.location.hostname.toLowerCase() === 'nashville.taxrescrm.app'"),'Nashville employee invites use the Nashville family-access bridge')
must(employees.includes("functionName = isNashville ? 'employee-access-link' : 'invite-employee'"),'employee invites route through the correct TaxRes-family backend')
must(inviteEmployee.includes("FAMILY_PASSWORD_PAGE='https://taxrescrm.app/family-password'"),'central employee invites use the TaxRes family password page')
...
must(stripeCheckout.includes("if (!usePlatformStripe && !connectedAccount)"),'unconnected prospect offices cannot charge through the primary Stripe account')
must(stripeCharge.includes("authClient.rpc('current_tenant_id')"),'saved-card charging resolves the authenticated office')
must(stripeCharge.includes("tenant_id:tenantId") || stripeCharge.includes("tenant_id: tenantId"),'payment results are recorded on the authenticated tenant')
must(stripeCharge.includes(".eq('tenant_id',tenantId)") || stripeCharge.includes(".eq('tenant_id', tenantId)"),'saved-card and customer lookups are tenant scoped')
...
must(stripeCharge.includes("const serviceRoleCall=token===service"),'saved-card charging preserves the authenticated server autopay path')
must(stripeCharge.includes("body?.tenant_id"),'server autopay must explicitly bind each charge to an office')
must(stripeCheckoutWebhook.includes("recordTenantId = record.tenant_id"),'checkout webhook resolves tenant from the actual lead/client record')
must(!stripeCheckoutWebhook.includes("tenant_id: '61a89aef-0e7e-4ea2-b222-44ab2024655a'"),'checkout webhook does not hard-code Tax Case Review for tenant writes')
must(stripeCheckoutWebhook.includes("tenant_id: lead.tenant_id"),'investigation-fee workflow tasks and notes stay on the lead tenant')
must(stripeCheckoutWebhook.includes("tenant_id: recordTenantId"),'checkout payment history stays on the record tenant')
must(stripeResolutionIntent.includes("'metadata[tenant_id]': String(lead.tenant_id)"),'resolution-fee PaymentIntent is bound to the lead tenant')
must(stripeResolutionConfirm.includes("if (!lead || lead.tenant_id !== tenantId)"),'resolution-fee confirmation rejects cross-tenant metadata')
must(!stripeResolutionConfirm.includes("|| '61a89aef-0e7e-4ea2-b222-44ab2024655a'"),'resolution-fee confirmation has no TCR fallback tenant')
must(autopayBatch.includes(".select('id,name,tenant_id,autopay_enabled"),'autopay batch carries tenant identity with every due client')
must(autopayBatch.includes("tenant_id:c.tenant_id"),'autopay invokes Stripe charging with explicit tenant scope')
must(autopayBatch.includes(".eq('tenant_id',tenantId)"),'autopay summary recipients are isolated by office')
must(autopayBatch.includes("Office payment processor not connected"),'autopay skips offices without a real payment processor rather than borrowing another merchant account')
