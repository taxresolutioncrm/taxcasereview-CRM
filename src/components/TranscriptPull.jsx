...
export default function TranscriptPull({ clientNames = [], clients = [], poas = [], onGoToPoa, onImported }) {
  ...
  async function createRequest() {
    ...
    try {
      ...
      setForm(BLANK)
      await loadRequests()
      flash(form.provider === 'irs_a2a' ? '✅ IRS TDS direct pull submitted. TaxRes CRM will retrieve and file delivered transcripts automatically.' : '✅ Manual TDS pull request created. The watched folder will file downloaded transcripts automatically.')
    } catch (e) {
      if (saved) {
        setModal(false)
        setForm(BLANK)
  ...
  return (
    <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
          <div>
            ...
            <div style={{ color: 'var(--t3)', fontSize: 11.5, marginTop: 4 }}>
              ...
              matched to the client by the name on the transcript, and securely filed with its analysis in TaxRes CRM.
      ...
      <div style={{ color: 'var(--t3)', fontSize: 11, marginTop: 10 }}>
        Direct requests auto-complete after the requested tax-year coverage is filed. Manual TDS remains available as the fallback path.
      </div>
