...
async function loadPlatformOfficeRowsFresh() {
  ...
  if (registrySyncJobs.length) {
    // Registry maintenance is a write-side synchronization concern, not a
    // prerequisite for rendering Overview. Keep it off the critical path so
    // slow writes cannot delay the portal's first useful paint.
...
function ProductReportingSelector({ value, onChange, channel, gscConnected, activeGscProduct, registryProducts, marketingConnectedProducts=[], marketingBlockedProducts=[], seoConnectedProducts=[], clarityConnectedProducts=[] }) {
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(145px,1fr))', gap:8, marginBottom:18 }}>
      {mapped.map(p => {
        ...
        return (
          <button key={p.key} onClick={() => onChange(p.key)} style={{
            minWidth:0, textAlign:'left', padding:'9px 10px', borderRadius:9, cursor:'pointer',
            ...
            <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:3, minWidth:0 }}>
              <span style={{ fontSize:12, flex:'0 0 auto' }}>{p.icon}</span>
              <span style={{ fontSize:12, fontWeight:800, lineHeight:1.15, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{p.label}</span>
            </div>
            <div style={{ fontSize:9, lineHeight:1.2, color:statusColor, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>● {statusLabel}</div>
...
function CommandCenter() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        // Start Sales + overview data together. The old serial prospect fetch
        // delayed the entire Command Center even though the queries are independent.
...
export default function AdminPortal() {
  ...
  // Do not boot SnappyMail on every Admin Portal visit. Mount it on the first
  // Email visit, then keep it mounted so drafts still survive tab switches.
  useEffect(() => {
  ...
  return (
    ...
    <div className="rl-admin-shell" style={{display:'flex',minHeight:'100vh',background:'#0d0c1a',fontFamily:'system-ui,Arial,sans-serif',width:'100%',overflowX:'hidden'}}>
      ...
      <div className="rl-admin-main" style={{flex:1,position:'relative',height:'100vh',overflowY:'auto'}}>
        {/* SnappyMail mounts only after Email is visited once, then remains
            persistent so compose drafts survive later tab switches. */}
