...
import FormaCorpFeePaymentModal from '../components/formacorp/FormaCorpFeePaymentModal'
...
export default function FormaCorp() {
  ...
  const [feePayCase, setFeePayCase] = useState(null)
  ...
  async function createFromWizard() {
    ...
    const payload = {
      ...
      stage:'Consultation', fee: isFloridaDirectFormation(wForm) ? floridaStateFee(wForm) : fee, fee_paid:false, ein:'', state_file_num:'', notes:'', formation_date:'', created_at:new Date().toISOString()
    }
  ...
  async function save() {
    ...
    if (isFloridaDirectFormation(payload)) {
  ...
  async function submitFloridaFax(c) {
    const missing = floridaSubmissionMissing(c)
    if (missing.length) { openFloridaEdit(c, missing); return }
    if (!['received'].includes(c.formation_funds_status || 'unpaid')) { showToast('Collect the state filing funds inside FormaCorp before submitting the filing.', 'err'); setFeePayCase(c); return }
    ...
    try {
      ...
      await updateFloridaCase(c, {
        ...
        fee_paid:true,
        state_disbursement_status:'prepaid_account',
        state_disbursement_reference:fax.sid || c.state_disbursement_reference || '',
  ...
  if (detail) {
    ...
    return (
      <div style={{maxWidth:900}}>
        ...
        {isFloridaDirectFormation(c) && (
          <div className="card" style={{padding:'12px 16px',marginBottom:10,borderLeft:`3px solid ${c.fl_filing_status==='Approved / Active'?'var(--ok)':flSubmitMissing.length?'var(--warn)':'var(--blue)'}`}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:0}}>
              {[['Principal Address',c.principal_address||'—'],['Mailing Address',c.mailing_address||c.principal_address||'—'],['Registered Agent',c.registered_agent||'—'],['Registered Agent Address',c.registered_agent_address||'—'],['Authorized Representative',c.authorized_representative||'—'],['Correspondence Email',c.correspondence_email||'—'],['Effective Date',c.effective_date||'Upon filing'],['State Filing Fee',`${Number(c.fl_state_fee || floridaStateFee(c)).toFixed(2)}`],['Funds Collected in CRM',c.formation_funds_status||'unpaid'],['Paid / Deducted by State',c.state_disbursement_status||'not_paid'],['Tracking #',c.fl_tracking_number||'—'],['Florida Document #',c.state_file_num||'—']].map(([l,v])=>(
                <div key={l} className="dr"><span className="dl">{l}</span><span className="dv">{v}</span></div>
              ))}
            </div>
            ...
            <div style={{display:'flex',gap:6,flexWrap:'wrap',marginTop:10}}>
              ...
              {c.formation_funds_status!=='received' && <button className="btn pri sm" onClick={()=>setFeePayCase(c)}>💳 Pay State Filing Funds</button>}
        ...
        {feePayCase && <FormaCorpFeePaymentModal caseRecord={feePayCase} onClose={()=>setFeePayCase(null)} onPaid={()=>{setFeePayCase(null);load()}}/>}
