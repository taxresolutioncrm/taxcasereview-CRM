...
import { buildFloridaFormationArticlesPdf, buildFlFaxPacket } from '../lib/flArticlesPdf'
...
const FL_FIELDS = {
  ...
  corporation_shares:'', corporation_par_value:'', incorporator_name:'', incorporator_address:'',
  officers_directors:'', articles_additional_provisions:'', state_fee_amount:'',
}
...
function floridaMissing(v = {}) {
  if (!isFloridaFormation(v)) return []
  const missing = []
  ...
  if (isFloridaCorporation(v)) {
    if (!String(v.incorporator_name || '').trim()) missing.push('Incorporator name')
    if (!String(v.incorporator_address || '').trim()) missing.push('Incorporator address')
    if (v.entity_type === 'C-Corp' && !(Number(v.corporation_shares) > 0)) missing.push('Authorized shares')
    if (v.entity_type === 'Non-Profit 501(c)(3)' && !String(v.business_purpose || '').trim()) missing.push('Nonprofit purpose')
  }
  return missing
}
...
function floridaStateFee(v = {}) {
  if (isFloridaCorporation(v)) return 70 + (v.fl_certificate_of_status ? 8.75 : 0) + (v.fl_certified_copy ? 8.75 : 0)
...
function FloridaFilingFields({ value, onChange }) {
  if (!isFloridaFormation(value)) return null
  return (
    <div className="card" style={{padding:'12px 14px',margin:'10px 0',background:'var(--s2)'}}>
      ...
      {isFloridaCorporation(value) && <div className="card" style={{padding:'10px 12px',margin:'8px 0',background:'var(--sf)'}}>
        <div style={{fontWeight:800,fontSize:11,marginBottom:8}}>Corporation Articles</div>
        <div className="fg2">
          <div className="field"><label>Incorporator Name *</label><input value={value.incorporator_name || ''} onChange={e=>onChange('incorporator_name',e.target.value)} placeholder="Legal name"/></div>
          <div className="field"><label>Incorporator Address *</label><input value={value.incorporator_address || ''} onChange={e=>onChange('incorporator_address',e.target.value)} placeholder="Street, city, state ZIP"/></div>
        </div>
        {value.entity_type === 'C-Corp' && <div className="fg2">
          <div className="field"><label>Authorized Shares *</label><input type="number" min="1" step="1" value={value.corporation_shares || ''} onChange={e=>onChange('corporation_shares',e.target.value)} placeholder="e.g. 1000"/></div>
          <div className="field"><label>Par Value (optional)</label><input type="number" min="0" step="0.0001" value={value.corporation_par_value || ''} onChange={e=>onChange('corporation_par_value',e.target.value)} placeholder="e.g. 0.01"/></div>
        </div>}
        <div className="field"><label>Officers / Directors (optional public listing)</label><textarea rows={2} value={value.officers_directors || ''} onChange={e=>onChange('officers_directors',e.target.value)} placeholder="Names, titles, and addresses as needed"/></div>
        <div className="field"><label>Additional Articles Provisions</label><textarea rows={3} value={value.articles_additional_provisions || ''} onChange={e=>onChange('articles_additional_provisions',e.target.value)} placeholder={value.entity_type==='Non-Profit 501(c)(3)'?'Include any required tax-exempt purpose/dissolution language for the organization.':'Optional additional provisions.'}/></div>
      </div>}
      <div className="field">
...
export default function FormaCorp() {
  ...
  async function downloadArticlesPdf(c) {
    ...
    setPdfBusy(true)
    try {
      const blob = await buildFloridaFormationArticlesPdf(c)
  ...
  async function createFromWizard() {
    ...
    const payload = {
      ...
      state_fee_amount:isFloridaFormation(wForm) ? floridaStateFee(wForm) : (wForm.state_fee_amount ? Number(wForm.state_fee_amount) : ((wForm.entity_type==='LLC'||wForm.entity_type==='Professional LLC (PLLC)') ? fee : null)),
      ...
      incorporator_address:wForm.incorporator_address || '',
      officers_directors:wForm.officers_directors || '',
      articles_additional_provisions:wForm.articles_additional_provisions || '',
      stage:'Consultation', fee: isFloridaFormation(wForm) ? floridaStateFee(wForm) : fee, fee_paid:false, ein:'', state_file_num:'', notes:'', formation_date:'', created_at:new Date().toISOString()
    }
  ...
  async function recordFloridaSubmission(c) {
    ...
    await updateFloridaCase(c, {
      ...
      state_fee_remittance_reference:tracking.trim(),
      fee_paid:true,
      stage:'State Filing',
  ...
  async function submitFloridaFax(c) {
    ...
    try {
      ...
      await updateFloridaCase(c, {
        ...
        state_fee_payment_status:'remitted',
        state_fee_remitted_at:new Date().toISOString(),
        state_fee_remittance_reference:fax.sid || c.fl_tracking_number || '',
        fee_paid:true,
        stage:'State Filing',
  ...
  async function updateStage(id, stage) {
    ...
    if (c && isFloridaFormation(c) && target >= STAGES.indexOf('State Filing')) {
      const missing = floridaMissing(c)
      if (missing.length) {
        openFloridaEdit(c, missing)
        return
    ...
    if (c && target >= STAGES.indexOf('EIN Application')) {
      const stateAccepted = isFloridaFormation(c)
    ...
    if (c && target >= STAGES.indexOf('Operating Agreement') && !String(c.ein || '').trim()) {
      showToast((c.entity_type==='C-Corp'||c.entity_type==='Non-Profit 501(c)(3)') ? 'Record the issued EIN before advancing to Governing Documents.' : 'Record the issued EIN before advancing to Operating Agreement.', 'err')
      return
    }
    if (c && target >= STAGES.indexOf('Bank Account Setup')) {
      ...
      if (!lifecycle || lifecycle.operating_agreement_status !== 'Signed') {
        showToast((c.entity_type==='C-Corp'||c.entity_type==='Non-Profit 501(c)(3)') ? 'The bylaws / organizational action must be signed before Bank Account Setup.' : 'The Operating Agreement must be signed before Bank Account Setup.', 'err')
        return
      }
        ...
        {isFloridaFormation(c) && (
          <div className="card" style={{padding:'12px 16px',marginBottom:10,borderLeft:`3px solid ${c.fl_filing_status==='Approved / Active'?'var(--ok)':flSubmitMissing.length?'var(--warn)':'var(--blue)'}`}}>
            ...
            <div style={{display:'flex',gap:6,flexWrap:'wrap',marginTop:10}}>
              ...
              {c.fl_payment_status!=='received' && c.fl_payment_status!=='state_paid' && c.fl_payment_status!=='state_account' && <button className="btn pri sm" onClick={()=>setFeePaymentCase(c)}>💳 Pay Government Filing Amount</button>}
              {(c.fl_payment_status==='received'||c.fl_payment_status==='state_paid'||c.fl_payment_status==='state_account') && <span className="bdg bg" style={{alignSelf:'center'}}>Government funds recorded</span>}
      ...
      {wizard && (
        <div className="modal-bg open" onClick={e=>e.target===e.currentTarget&&setWizard(false)}>
          <div className="modal" style={{width:560,maxHeight:'90vh',overflowY:'auto'}}>
            ...
            {wStep===5 && <div><div style={{fontSize:13,color:'var(--t3)',marginBottom:14}}>Review the details below, then create the formation case and lifecycle.</div><div className="card" style={{padding:'14px 16px',background:'var(--s2)',marginBottom:10}}>{[['Service Plan',wForm.service_plan||'Launch'],['Entity Type',`${ENTITY_ICONS[wForm.entity_type]||'🏢'} ${wForm.entity_type}`],['State',`${stateReqs[wForm.state]?.state_name||wForm.state} (${wForm.state})`],['Client',wForm.client_name],['Entity Name',wForm.entity_name],['Owners/Members',wForm.owners||'—'],['Registered Agent',wForm.registered_agent||'—'],['Business Purpose',wForm.business_purpose||'—']].map(([l,v])=><div key={l} className="dr"><span className="dl">{l}</span><span className="dv">{v}</span></div>)}</div>{isFloridaFormation(wForm)&&<div className="card" style={{padding:'14px 16px',background:'var(--s2)',marginBottom:10}}><div style={{fontWeight:700,fontSize:13,marginBottom:8}}>☀️ Florida Filing Readiness</div>{floridaSubmissionMissing(wForm).length===0?<div style={{fontSize:12,color:'var(--ok)'}}>✅ Florida submission details and filing authorization are ready.</div>:<div style={{fontSize:12,color:'var(--warn)',lineHeight:1.5}}>Case can be created as a Consultation draft. Before state submission, complete: {floridaSubmissionMissing(wForm).join(', ')}.</div>}</div>}{stateReqs[wForm.state]&&<div className="card" style={{padding:'14px 16px',background:'var(--s2)'}}><div style={{fontWeight:700,fontSize:13,marginBottom:8}}>📍 {stateReqs[wForm.state].state_name} Filing Snapshot</div><div className="dr"><span className="dl">Government Filing Amount</span><span className="dv">{stateFilingAmount(wForm,stateReqs[wForm.state])?('<div className="dr"><span className="dl">Processing Time</span><span className="dv">{stateReqs[wForm.state].processing_time}</span></div><div className="dr"><span className="dl">Annual Report</span><span className="dv">{stateReqs[wForm.state].annual_report_fee}</span></div></div>}<div style={{fontSize:12,color:'var(--t3)',marginTop:10,lineHeight:1.6}}>This creates a formation case starting at the <strong>Consultation</strong> stage. You'll be taken to the case page where you can track progress through filing, EIN, operating agreement, and more.</div></div>}

            <div style={{display:'flex',justifyContent:'space-between',gap:10,marginTop:20,paddingTop:16,borderTop:'1px solid var(--br)'}}><button className="btn" onClick={()=>wStep===1?setWizard(false):setWStep(s=>s-1)}>{wStep===1?'Cancel':'← Back'}</button>{wStep<5?<button className="btn pri" disabled={(wStep===1&&!wForm.entity_type)||(wStep===2&&(!wForm.state||(!isFloridaFormation(wForm)&&wForm.entity_type!=='LLC'&&wForm.entity_type!=='Professional LLC (PLLC)'&&!(Number(wForm.state_fee_amount)>0))))||(wStep===3&&(!wForm.client_name||!wForm.entity_name))} onClick={()=>setWStep(s=>s+1)}>Continue →</button>:<button className="btn pri" onClick={createFromWizard} disabled={saving}>{saving?'Creating…':'🏢 Create Formation + Lifecycle'}</button>}</div>
          </div>
        </div>
      )}
    </div>
...
+floridaStateFee(wForm).toFixed(2)}</span></div>}<div className="dr"><span className="dl">Processing Time</span><span className="dv">{stateReqs[wForm.state].processing_time}</span></div><div className="dr"><span className="dl">Annual Report</span><span className="dv">{stateReqs[wForm.state].annual_report_fee}</span></div>{!isFloridaFormation(wForm)&&wForm.entity_type!=='LLC'&&wForm.entity_type!=='Professional LLC (PLLC)'&&<div className="field" style={{marginTop:10}}><label>Current Government Filing Amount *</label><input type="number" min="0" step="0.01" value={wForm.state_fee_amount||''} onChange={e=>wFld('state_fee_amount',e.target.value)} placeholder="Enter the current official state filing amount"/><div style={{fontSize:10,color:'var(--t3)',marginTop:4}}>Use the current amount published by the state for this entity type. FormaCorp collects this exact amount without adding a formation-provider fee.</div></div>}{stateReqs[wForm.state].notes&&<div style={{fontSize:12,color:'var(--t3)',marginTop:8,paddingTop:8,borderTop:'1px solid var(--br)',lineHeight:1.5}}>ℹ️ {stateReqs[wForm.state].notes}</div>}</div>}</div>}
...
+Number(stateFilingAmount(wForm,stateReqs[wForm.state])).toFixed(2)):'—'}</span></div><div className="dr"><span className="dl">Processing Time</span><span className="dv">{stateReqs[wForm.state].processing_time}</span></div><div className="dr"><span className="dl">Annual Report</span><span className="dv">{stateReqs[wForm.state].annual_report_fee}</span></div></div>}<div style={{fontSize:12,color:'var(--t3)',marginTop:10,lineHeight:1.6}}>This creates a formation case starting at the <strong>Consultation</strong> stage. You'll be taken to the case page where you can track progress through filing, EIN, operating agreement, and more.</div></div>}
