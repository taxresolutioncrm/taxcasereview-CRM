...
export default function FormaCorp() {
  ...
  async function createFromWizard() {
    ...
    const flError = validateFloridaFiling(wForm)
    if (flError) { showToast(flError, 'err'); setWStep(3); return }
    setSaving(true)
    ...
    const payload = {
      ...
      principal_address: wForm.principal_address,
      mailing_address: wForm.mailing_address,
      registered_agent_address: wForm.registered_agent_address,
      authorized_representative: wForm.authorized_representative,
      authorized_representative_title: wForm.authorized_representative_title,
      correspondence_email: wForm.correspondence_email,
      effective_date: wForm.effective_date,
  ...
  async function save() {
    ...
    const flError = validateFloridaFiling(form)
  ...
  async function updateStage(id, stage) {
    ...
    if (error) { showToast('Could not update stage: '+error.message, 'err'); return }
        ...
        {lookup.open && (
          <div className="modal-bg open" onClick={e=>e.target===e.currentTarget&&setLookup({ open:false, query:'', running:false, result:null })}>
            <div className="modal" style={{maxWidth:520}}>
              ...
              <div style={{display:'flex',gap:6,marginBottom:10}}><input value={lookup.query} onChange={e=>setLookup(l=>({ ...l, query:e.target.value }))} onKeyDown={e=>e.key==='Enter'&&runLookup()} placeholder="e.g. Smith Holdings LLC or L23000012345" style={{flex:1,padding:'8px 10px',background:'var(--s2)',border:'1px solid var(--br)',borderRadius:6,color:'var(--tx)',fontSize:13}}/><button className="btn pri" onClick={runLookup} disabled={lookup.running || !lookup.query || lookup.query.length < 3}>{lookup.running ? '…' : 'Search'}</button></div>
      ...
      {modal && (
        <div className="modal-bg open" onClick={e=>e.target===e.currentTarget&&setModal(false)}>
          <div className="modal" style={{width:620,maxHeight:'90vh',overflowY:'auto'}}>
            ...
            <div style={{position:'relative'}} className="field"><label>Client *</label><input value={form.client_name} onChange={e=>searchClient(e.target.value)} placeholder="Search client…"/>{showSug && sugg.length>0 && <div style={{position:'absolute',top:'100%',left:0,right:0,background:'var(--sf)',border:'1px solid var(--br)',borderRadius:6,zIndex:50,maxHeight:160,overflowY:'auto'}}>{sugg.map(c=><div key={c.id} onClick={()=>{fld('client_name',c.name);setSugg([]);setShowSug(false)}} style={{padding:'8px 12px',cursor:'pointer',fontSize:13}}>{c.name}</div>)}</div>}</div>
            <div className="fg2"><div className="field"><label>Entity Name *</label><input value={form.entity_name} onChange={e=>fld('entity_name',e.target.value)} placeholder="e.g. Smith Holdings LLC"/></div><div className="field"><label>Entity Type *</label><select value={form.entity_type} onChange={e=>fld('entity_type',e.target.value)}>{ENTITY_TYPES.map(t=><option key={t}>{t}</option>)}</select></div></div>
            <div className="fg2"><div className="field"><label>State of Formation</label><select value={form.state} onChange={e=>fld('state',e.target.value)}>{STATES.map(s=><option key={s}>{s}</option>)}</select></div><div className="field"><label>Formation Date</label><input type="date" value={form.formation_date||''} onChange={e=>fld('formation_date',e.target.value)}/></div></div>
            ...
            {form.state === 'FL' && <>
              <div className="field"><label>Principal Office Street Address *</label><input value={form.principal_address||''} onChange={e=>fld('principal_address',e.target.value)} placeholder="Street, city, FL ZIP"/></div>
              <div className="field"><label>Mailing Address</label><input value={form.mailing_address||''} onChange={e=>fld('mailing_address',e.target.value)} placeholder="Leave blank if same as principal office"/></div>
              <div className="field"><label>Registered Agent Florida Street Address *</label><input value={form.registered_agent_address||''} onChange={e=>fld('registered_agent_address',e.target.value)} placeholder="Street, city, FL ZIP — no P.O. box"/></div>
              <div className="fg2"><div className="field"><label>Authorized Representative *</label><input value={form.authorized_representative||''} onChange={e=>fld('authorized_representative',e.target.value)} placeholder="Full legal name"/></div><div className="field"><label>Representative Title</label><select value={form.authorized_representative_title||'AMBR'} onChange={e=>fld('authorized_representative_title',e.target.value)}><option value="AMBR">AMBR — Authorized Member</option><option value="MGRM">MGRM — Managing Member</option><option value="MGR">MGR — Manager</option></select></div></div>
              <div className="fg2"><div className="field"><label>Correspondence Email *</label><input type="email" value={form.correspondence_email||''} onChange={e=>fld('correspondence_email',e.target.value)} placeholder="filing contact email"/></div><div className="field"><label>Requested Effective Date</label><input type="date" value={form.effective_date||''} onChange={e=>fld('effective_date',e.target.value)}/></div></div>
              <div className="field"><label style={{display:'flex',alignItems:'flex-start',gap:8,cursor:'pointer'}}><input type="checkbox" checked={!!form.registered_agent_accepted} onChange={e=>fld('registered_agent_accepted',e.target.checked)} style={{width:'auto',marginTop:2}}/><span>Registered agent has accepted the appointment *</span></label></div>
            </>}
        ...
        <div className="modal-bg open" onClick={e=>e.target===e.currentTarget&&setWizard(false)}>
          <div className="modal" style={{width:620,maxHeight:'90vh',overflowY:'auto'}}>
            ...
            {wStep === 3 && <div>
              ...
              {wForm.state === 'FL' && <>
                <div className="field"><label>Principal Office Street Address *</label><input value={wForm.principal_address} onChange={e=>wFld('principal_address',e.target.value)} placeholder="Street, city, FL ZIP"/></div>
                <div className="field"><label>Mailing Address</label><input value={wForm.mailing_address} onChange={e=>wFld('mailing_address',e.target.value)} placeholder="Leave blank if same as principal office"/></div>
                <div className="field"><label>Registered Agent Florida Street Address *</label><input value={wForm.registered_agent_address} onChange={e=>wFld('registered_agent_address',e.target.value)} placeholder="Street, city, FL ZIP — no P.O. box"/></div>
                <div className="fg2"><div className="field"><label>Authorized Representative *</label><input value={wForm.authorized_representative} onChange={e=>wFld('authorized_representative',e.target.value)} placeholder="Full legal name"/></div><div className="field"><label>Representative Title</label><select value={wForm.authorized_representative_title} onChange={e=>wFld('authorized_representative_title',e.target.value)}><option value="AMBR">AMBR — Authorized Member</option><option value="MGRM">MGRM — Managing Member</option><option value="MGR">MGR — Manager</option></select></div></div>
                <div className="fg2"><div className="field"><label>Correspondence Email *</label><input type="email" value={wForm.correspondence_email} onChange={e=>wFld('correspondence_email',e.target.value)} placeholder="filing contact email"/></div><div className="field"><label>Requested Effective Date</label><input type="date" value={wForm.effective_date} onChange={e=>wFld('effective_date',e.target.value)}/></div></div>
                <div className="field"><label style={{display:'flex',alignItems:'flex-start',gap:8,cursor:'pointer'}}><input type="checkbox" checked={wForm.registered_agent_accepted} onChange={e=>wFld('registered_agent_accepted',e.target.checked)} style={{width:'auto',marginTop:2}}/><span>I confirm the registered agent has accepted the appointment *</span></label></div>
              </>}
