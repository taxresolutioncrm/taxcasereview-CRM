...
export default function FormaCorp() {
  ...
  const [flSubmit, setFlSubmit] = useState({ method:'sunbiz_online', faxNumber:'', coverSheet:null, busy:false })
  ...
  async function submitFloridaFax(c) {
    ...
    try {
      const packet = await buildFlFaxPacket(flSubmit.coverSheet, c)
        ...
        {isFloridaLlc(c) && (
          <div className="card" style={{padding:'12px 16px',marginBottom:10,borderLeft:`3px solid ${c.fl_filing_status==='Approved / Active'?'var(--ok)':flSubmitMissing.length?'var(--warn)':'var(--blue)'}`}}>
            ...
            <div style={{marginTop:12,paddingTop:10,borderTop:'1px solid var(--br)'}}>
              ...
              <div style={{fontSize:10,color:'var(--t3)',lineHeight:1.5,marginBottom:8}}>For a frequent-filer Sunbiz account: generate the official Electronic Filing Cover Sheet in Sunbiz, attach it here, enter the fax number printed on that cover sheet, and FormaCorp will combine it with the Articles and send the filing through the CRM fax service.</div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                ...
                <input type="file" accept="application/pdf" onChange={e=>setFlSubmit(x=>({...x,coverSheet:e.target.files?.[0]||null}))} style={{width:'100%',fontSize:11}}/>
              </div>
